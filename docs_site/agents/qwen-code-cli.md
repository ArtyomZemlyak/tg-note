...
- **Docling MCP**: Document processing (PDF, DOCX, images, OCR)
  ...
  - Accessible via `convert_document_from_content` tool
...
2. Qwen CLI reads the configuration on startup
3. CLI automatically connects to configured MCP servers
4. All MCP tools become available in CLI sessions

For more details, see [MCP Architecture](../architecture/mcp-architecture.md) and [File Format Recognition](../user-guide/file-format-recognition.md).
