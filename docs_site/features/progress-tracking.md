...
Progress messages show (v0.2.2+):

```
🔄 Обрабатываю контент... (45%)

instruction_v5
✅ Инструкция прочитана целиком
✅ Язык
✅ Определение типа источника
⏳ Многоэтапный поиск в базе знаний
⬜ Глубокий анализ
...
- All tasks shown (no "...и еще N задач")
- Clean task names without service keywords (КРИТИЧНО, ###, Шаг N:)
- Grouped by file for better hierarchy
- Shorter, more readable task descriptions
...
### Text Cleaning (v0.2.2+)

Checkbox text is cleaned for better readability:
...
# Remove: Шаг N:, Этап N:
text = re.sub(r"^(Шаг|Этап)\s+\d+:\s*", "", text)

# Remove: КРИТИЧНО, ВАЖНО, ОБЯЗАТЕЛЬНО
text = re.sub(r"^(КРИТИЧНО|ВАЖНО|ОБЯЗАТЕЛЬНО|ВНИМАНИЕ)[:.\s]*", "", text)
...
**Examples:**
- `### Шаг 1: Анализ` → `Анализ`
- `КРИТИЧНО: Обработать файлы` → `Обработать файлы`
- `[Инструкция](path.md) прочитана` → `Инструкция прочитана`
