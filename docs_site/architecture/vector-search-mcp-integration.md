...
2. **`reindex_vector`** - Реиндексация базы знаний
   - `force` (bool) - Принудительная реиндексация (default: false)
   - `user_id` (int, optional) - ID пользователя
...
1. **Мониторинг изменений** (каждые 5 минут)
   ```
...
2. **Триггер реиндексации**
   ```
   При обнаружении изменений:
   → Сохранение новых хешей
   → VectorSearchManager автоматически индексирует только изменения
      (при следующем вызове reindex_vector)
   ```
