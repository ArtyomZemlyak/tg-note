...
async def example_kb_read_file(agent: AutonomousAgent):
    ...
    result = await agent._tool_kb_read_file({
        "paths": ["topics/ai/neural-networks.md"]
    })
    ...
    result = await agent._tool_kb_read_file({
        "paths": [
            "topics/ai/neural-networks.md",
            "topics/tech/python.md"
        ]
    })
...
async def example_combined_workflow(agent: AutonomousAgent):
    ...
    list_result = await agent._tool_kb_list_directory({
        "path": "topics/ai",
        "recursive": False
    })
    ...
    if search_result["success"]:
        ...
        if search_result["matches"]:
            ...
            read_result = await agent._tool_kb_read_file({
                "paths": file_paths
            })
...
async def main():
    ...
    agent = AutonomousAgent(
        llm_connector=None,  # No LLM needed for these examples
