...
class MemoryStorage(BaseMemoryStorage):
    ...
    def __init__(
        self, data_dir: Path, storage_type: Optional[str] = None, model_name: Optional[str] = None
    ):
        """
        ...
        try:
            self._storage = MemoryStorageFactory.create(
                storage_type=storage_type, data_dir=data_dir, model_name=model_name
            )
            logger.info(f"[MemoryStorage] Initialized with '{storage_type}' storage at {data_dir}")
            ...
            self._storage = MemoryStorageFactory.create(storage_type="json", data_dir=data_dir)
