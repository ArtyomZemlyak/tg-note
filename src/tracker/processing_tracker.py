...
class ProcessingTracker:
    ...
    def _ensure_storage(self) -> None:
        ...
        if not self.storage_path.exists():
            self._write_data({"processed_messages": [], "pending_groups": []})
    ...
    def remove_pending_group(self, group_id: str) -> None:
        ...
        data["pending_groups"] = [
            g for g in data["pending_groups"] if g.get("group_id") != group_id
        ]
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "completed": len(
                [m for m in data["processed_messages"] if m.get("status") == "completed"]
            ),
            "failed": len([m for m in data["processed_messages"] if m.get("status") == "failed"]),
        }
