    ...
    def _suggest_correct_path(self, img_path: str, markdown_path: Path) -> Optional[str]:
        ...
        if self.images_dir.exists():
            for img_file in self.images_dir.iterdir():
                if img_file.name == filename and img_file.is_file():
                    # Calculate correct relative path from markdown to image
                    try:
                        rel_path = Path(img_file).relative_to(markdown_path.parent)
                        return str(rel_path)
                    except ValueError:
                        # Can't create relative path, use absolute from KB root
                        try:
                            rel_from_kb = img_file.relative_to(self.kb_root)
                            return str(rel_from_kb)
                        except ValueError:
                            pass
