"""
Markdown Media Path Validator
Validates media references (primarily images) in markdown files generated by agents.
"""
...
class MediaReference:
    """Represents a media reference found in markdown."""
    ...
    def __repr__(self) -> str:
        return (
            f"MediaReference(path='{self.path}', line={self.line_number}, "
...
class ValidationIssue:
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"at {self.media_ref.markdown_file.name}:{self.media_ref.line_number})"
        )


class MarkdownMediaValidator:
    """
    ...
    def validate_markdown_file(
    ...
    ) -> Tuple[List[MediaReference], List[ValidationIssue]]:
        """
        ...
        image_refs: List[MediaReference] = []
    ...
    def _validate_alt_text(self, alt_text: str, img_ref: MediaReference) -> List[ValidationIssue]:
        """
    ...
    def generate_report(
        ...
        if not issues_by_file:
            ...
            for file_path, issues in sorted(issues_by_file.items()):
                ...
                for issue in issues:
                    ...
                    lines.append(f"  {icon} Line {issue.media_ref.line_number}: {issue.message}")
                    lines.append(f"     Image path: {issue.media_ref.path}")
                    if issue.suggestion:
        ...
        if verbose:
            lines.append(f"ℹ️  Info:     {total_info}")
        lines.append("")
...
def validate_agent_generated_markdown(
    ...
    if errors:
        ...
        for error in errors:
            logger.error(f"  Line {error.media_ref.line_number}: {error.message}")
            if error.suggestion:
    ...
    if warnings:
        ...
        for warning in warnings:
            logger.warning(f"  Line {warning.media_ref.line_number}: {warning.message}")

    logger.info(f"Image validation passed for {markdown_file.name}")
    return True


def validate_kb_media(kb_root: Path, verbose: bool = False) -> int:
    """
...
def validate_kb_images(kb_root: Path, verbose: bool = False) -> int:
    return validate_kb_media(kb_root, verbose=verbose)
