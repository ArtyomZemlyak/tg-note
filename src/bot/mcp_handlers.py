...
class MCPHandlers:
            ...
            if server.enabled:
                keyboard.add(
                    InlineKeyboardButton(
                        f"🔴 Disable {server.name}",
                        callback_data=f"mcp:disable:{server.name}"
                    )
                )
            else:
                keyboard.add(
                    InlineKeyboardButton(
                        f"🟢 Enable {server.name}",
                        callback_data=f"mcp:enable:{server.name}"
                    )
                )
    ...
    async def handle_remove_mcp(self, message: Message) -> None:
        ...
        keyboard.add(
            InlineKeyboardButton(
                "✅ Yes, delete",
                callback_data=f"mcp:confirm_remove:{server_name}"
            ),
            InlineKeyboardButton(
                "❌ Cancel",
                callback_data="mcp:cancel_remove"
            )
        )
        ...
        try:
            ...
            if action == "list":
                ...
                await self.bot.send_message(
                    ...
                    "Use /addmcpserver to add a new MCP server configuration."
                )
                ...
                if success:
                    ...
                    await self.bot.answer_callback_query(
                        call.id,
                        f"❌ Failed to enable {server_name}",
                        show_alert=True
                    )
                ...
                if success:
                    ...
                    await self.bot.answer_callback_query(
                        call.id,
                        f"❌ Failed to disable {server_name}",
                        show_alert=True
                    )
                ...
                if success:
                    ...
                    await self.bot.answer_callback_query(
                        call.id,
                        f"❌ Failed to delete {server_name}",
                        show_alert=True
                    )
    ...
    async def _refresh_server_list(self, call: CallbackQuery) -> None:
        ...
        try:
            ...
            if "can't parse entities" in str(e).lower():
                logger.warning(f"Markdown parsing failed in MCP list, sending without formatting: {e}")
                await self.bot.edit_message_text(
                    text,
                    call.message.chat.id,
                    call.message.message_id,
