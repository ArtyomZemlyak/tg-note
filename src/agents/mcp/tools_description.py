...
async def get_mcp_tools_description(
    ...
    try:
        ...
        lines.append("")
        lines.append("The following MCP (Model Context Protocol) servers are available with their tools:")
        lines.append("")
        ...
        for server_name, client in connected_clients.items():
            try:
                ...
                logger.error(f"[MCPToolsDescription] Failed to describe tools from {server_name}: {e}")
        ...
        lines.append("---")
        lines.append("")
        lines.append(f"**Total MCP tools available**: {total_tools} from {len(connected_clients)} server(s)")
        lines.append("")
        lines.append("**Usage**: Call these tools using their agent tool name (e.g., `mcp_server_tool`)")
        lines.append("")
