...
class ChatWithMemoryTool(BaseMCPTool):
    ...
    def _get_or_create_agent(self, memory_path: str, use_vllm: bool = True) -> Agent:
...
class QueryMemoryAgentTool(BaseMCPTool):
    ...
    def parameters_schema(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Your query to search for in memory"}
            },
            "required": ["query"],
        }
    ...
    async def execute(self, params: Dict[str, Any], context: "ToolContext") -> Dict[str, Any]:
        ...
        if not MEM_AGENT_AVAILABLE:
            return {"success": False, "error": "mem-agent is not available"}
        ...
        try:
            ...
            return await chat_tool.execute({"message": query_message, "use_vllm": True}, context)
