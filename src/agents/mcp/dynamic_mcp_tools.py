...
class DynamicMCPTool(BaseTool):
    ...
    def __init__(
        ...
        self._parameters = tool_schema.get("inputSchema", {
            "type": "object",
            "properties": {},
            "required": []
        })
...
async def discover_and_create_mcp_tools(
    ...
    try:
        ...
        for server_name, client in connected_clients.items():
            try:
                ...
                for tool_schema in available_tools:
                    ...
                    if not tool_name:
                        logger.warning(f"[DynamicMCPTools] Skipping tool with no name from {server_name}")
                        continue
        ...
        logger.info(f"[DynamicMCPTools] Created {len(tools)} MCP tools from {len(connected_clients)} servers")
