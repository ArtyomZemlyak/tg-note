...
class AutonomousAgent(BaseAgent):
    ...
    async def get_mcp_tools_description(self) -> str:
        ...
        try:
            ...
            if formatted:
                logger.info(
                    f"[AutonomousAgent] MCP tools description generated ({len(formatted)} chars)"
                )
...
"""
    ...
    async def _make_decision_llm(self, context: AgentContext) -> AgentDecision:
        ...
        if context.executions:
            ...
            messages.append({"role": "assistant", "content": f"История выполнения:\n{history}"})
        ...
        try:
            ...
            response = await self.llm_connector.chat_completion(
                messages=messages, tools=tools_schema, temperature=0.7
            )
            ...
            if response.content:
                ...
                return AgentDecision(
                    action=ActionType.END, reasoning="Task completed", final_result=response.content
                )
    ...
    async def _execute_tool(self, decision: AgentDecision) -> ToolExecution:
        ...
        if not self.tool_manager.has(tool_name):
            ...
            return ToolExecution(
                tool_name=tool_name, params=params, result=None, success=False, error=error
            )
        ...
        try:
            ...
            return ToolExecution(tool_name=tool_name, params=params, result=result, success=success)
    ...
    async def _finalize_result(
        self, context: AgentContext, decision: Optional[AgentDecision], iterations: int
        ...
        metadata = {
            ...
            "llm_model": (
                self.llm_connector.get_model_name() if self.llm_connector else "rule-based"
            ),
    ...
    async def _determine_kb_structure_from_markdown(
        self, markdown: str, context: AgentContext
    ) -> KBStructure:
        """
    ...
    async def _structure_content(
        self, content: Dict, execution_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
