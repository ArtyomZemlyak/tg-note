...
class MessageProcessor(IMessageProcessor):
    ...
    async def process_message(self, message: IncomingMessageDTO) -> None:
        ...
        try:
            ...
            processing_msg = await self.bot.send_message(
                message.chat_id, "🔄 Обрабатываю сообщение..."
            )

            # Convert DTO to dict format for aggregator
            message_dict = MessageMapper.to_dict(message)

            # Get user-specific aggregator (thread-safe)
            user_aggregator = await self.user_context_manager.get_or_create_aggregator(
                message.user_id
            )
            ...
            if closed_group:
                ...
                prev_processing_msg = await self.bot.send_message(
                    message.chat_id, "🔄 Обрабатываю предыдущую группу сообщений..."
                )
                await self.process_message_group(
                    closed_group, prev_processing_msg.message_id, message.chat_id
                )
    ...
    async def process_message_group(
        ...
        try:
            ...
            elif user_mode == UserMode.AGENT.value:
