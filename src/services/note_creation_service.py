    ...
    async def _create_note_locked(
        ...
        if git_ops.enabled and user_kb.get("kb_type") == "github":
            ...
            try:
                ...
                self.logger.info(
                    f"[NOTE_SERVICE] Creating log_callback for message_id={log_message_id}, "
                ...
                async def update_log_message(log_snippet: str) -> None:
                    ...
                    try:
                        ...
                        if last_sent_text[0] == log_text:
                            self.logger.debug(
                                f"[NOTE_SERVICE] Log text unchanged, skipping update ({len(log_snippet)} chars)"
                            )
                            return

                        self.logger.info(
                            f"[NOTE_SERVICE] Updating log message with {len(log_snippet)} chars"
                        )
                        ...
                        if success:
                            ...
                            self.logger.debug(f"[NOTE_SERVICE] Log message updated successfully")
                        else:
                            self.logger.warning(
                                f"[NOTE_SERVICE] Failed to update log message (edit returned False)"
                            )
                    except Exception as e:
                ...
                if len(processing_message_ids) > 2:
                    ...
                    self.logger.info(
                        f"[NOTE_SERVICE] Creating error_callback for message_id={error_message_id}"
                    )
                    ...
                    async def update_error_message(error_msg: str) -> None:
                        ...
                        try:
                            ...
                            if success:
                                self.logger.debug(
                                    f"[NOTE_SERVICE] Error message updated successfully"
                                )
                        except Exception as e:
                            self.logger.warning(
                                f"Failed to update error message: {e}", exc_info=True
                            )
                ...
                self.logger.info(
                    f"[NOTE_SERVICE] Passing note_content to agent: "
                    f"has_log_callback={log_callback is not None}, "
                    f"log_chars={note_content.get('log_chars')}, "
                    f"log_update_interval={note_content.get('log_update_interval')}"
                )
