...
class QuestionAnsweringService(BaseKBService, IQuestionAnsweringService):
    ...
    async def answer_question(
        ...
        try:
            ...
            self.logger.info(
                f"[ASK_SERVICE] Creating log_callback for message_id={log_message_id}, "
            ...
            async def update_log_message(log_snippet: str) -> None:
                ...
                try:
                    ...
                    if last_sent_text[0] == log_text:
                        self.logger.debug(
                            f"[ASK_SERVICE] Log text unchanged, skipping update ({len(log_snippet)} chars)"
                        )
                        return

                    self.logger.info(
                        f"[ASK_SERVICE] Updating log message with {len(log_snippet)} chars"
                    )
                    ...
                    if success:
                        ...
                        self.logger.debug(f"[ASK_SERVICE] Log message updated successfully")
                    else:
                        self.logger.warning(
                            f"[ASK_SERVICE] Failed to update log message (edit returned False)"
                        )
            ...
            if len(processing_message_ids) > 2:
                ...
                self.logger.info(
                    f"[ASK_SERVICE] Creating error_callback for message_id={error_message_id}"
                )
                ...
                async def update_error_message(error_msg: str) -> None:
                    ...
                    try:
                        ...
                        if success:
                            self.logger.debug(f"[ASK_SERVICE] Error message updated successfully")
                    except Exception as e:
            ...
            processed_content = await self._query_kb(
                kb_path,
                question_text,
                user_id,
    ...
    async def _query_kb(
        ...
        Args:
            ...
            user_id: User ID
            log_callback: Optional async callback function to receive log snippets
            chat_id: Chat ID for log updates (optional)
        ...
        self.logger.info(
            f"[ASK_SERVICE] Passing query_content to agent: "
            f"has_log_callback={log_callback is not None}, "
            f"log_chars={query_content.get('log_chars')}, "
            f"log_update_interval={query_content.get('log_update_interval')}"
        )
