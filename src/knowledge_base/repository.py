    ...
    def clone_github_kb(
        ...
        try:
            ...
            if credentials and credentials.get('username') and credentials.get('token'):
                clone_url = self._inject_credentials(github_url, credentials['username'], credentials['token'])
    ...
    def pull_updates(self, kb_path: Path) -> Tuple[bool, str, Optional[Path]]:
        ...
        try:
            ...
            if repo.is_dirty():
                ...
                return False, "Repository has uncommitted changes. Commit or stash them first.", kb_path
