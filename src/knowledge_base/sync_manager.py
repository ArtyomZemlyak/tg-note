...
class _KBLockContext:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(None, self.file_lock.acquire)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.file_lock and self.file_lock.is_locked:
            try:
                await asyncio.get_event_loop().run_in_executor(None, self.file_lock.release)
                logger.info(
                    f"[{self.operation_name}] Released file lock for KB: {kb_path_normalized}"
                )
            except Exception as e:
