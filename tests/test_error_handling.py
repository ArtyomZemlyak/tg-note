...
"""
    ...
    async def test_execute_cli_non_zero_exit(self, agent):
        ...
        mock_process.communicate = AsyncMock(return_value=(b"", b"Error: authentication required"))
    ...
    async def test_process_with_fallback_error(self, agent):
        ...
        with patch("asyncio.create_subprocess_exec", return_value=mock_process):
            ...
            with patch.object(
                agent, "_fallback_processing", side_effect=Exception("Fallback failed")
            ):
                with pytest.raises(RuntimeError) as exc_info:
...
class TestKnowledgeBaseManagerErrorHandling:
    ...
    def test_create_article_none_content(self, kb_manager):
        ...
        with pytest.raises(ValueError) as exc_info:
            kb_manager.create_article(content=None, title="Test", kb_structure=kb_structure)
    ...
    def test_create_article_empty_content(self, kb_manager):
        ...
        with pytest.raises(ValueError) as exc_info:
            kb_manager.create_article(content="", title="Test", kb_structure=kb_structure)
    ...
    def test_create_article_none_title(self, kb_manager):
        ...
        with pytest.raises(ValueError) as exc_info:
            kb_manager.create_article(content="Test content", title=None, kb_structure=kb_structure)
    ...
    def test_create_article_none_kb_structure(self, kb_manager):
        ...
        with pytest.raises(ValueError) as exc_info:
            kb_manager.create_article(content="Test content", title="Test", kb_structure=None)
    ...
    def test_create_article_none_category(self, kb_manager):
        ...
        with pytest.raises(ValueError) as exc_info:
            kb_manager.create_article(
                content="Test content", title="Test", kb_structure=kb_structure
            )
    ...
    def test_create_article_valid(self, kb_manager):
        ...
        result = kb_manager.create_article(
            content="Test content", title="Test Article", kb_structure=kb_structure
        )
